  ...
  return (
    <div className="max-w-7xl mx-auto selection:bg-primary/20 selection:text-text pb-12">
      <section className="mb-12">
        <h2 className="font-headline text-4xl md:text-5xl font-extrabold text-text tracking-tight mb-4">Artisan Shop</h2>
        <p className="text-gray-500 max-w-2xl text-lg leading-relaxed">
          Explore handcrafted architectural pieces curated for the modern greenhouse. Each item tells a story of organic structure and professional precision.
        </p>
      </section>

      {loading && <div className="p-10 border border-gray-200 rounded-2xl text-center shadow-sm bg-white animate-pulse text-primary font-bold mb-10">Loading products...</div>}
      
      {!loading && products.length === 0 && (
        <div className="p-10 border border-gray-200 rounded-2xl text-center bg-white shadow-sm mb-10">
          <p className="text-gray-500">No products are currently available in the shop.</p>
        </div>
      )}
            ...
            <div className="aspect-[4/5] relative overflow-hidden bg-gray-100 flex justify-center items-center">
              {product.imageUrl ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 group-hover:scale-105 transition-transform duration-700">
                  ...
                  <span className="text-sm font-semibold tracking-wider uppercase opacity-70">Image Coming Soon</span>
                </div>
              )}
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full shadow-sm">
                <span className="text-[10px] font-bold uppercase tracking-wider text-primary">Limited Edition</span>
              </div>
            </div>
            <div className="p-6 flex flex-col flex-grow">
              <span className="font-label text-xs font-semibold text-gray-500 mb-1 uppercase tracking-widest">
                {product.ownerName || 'Artisan Seller'}
              </span>
