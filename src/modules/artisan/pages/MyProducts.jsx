  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-12">
      <div className="mb-8 flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-headline font-extrabold text-text tracking-tight">My Products</h1>
          <p className="text-gray-500 font-body mt-2">{loading ? '—' : `${products.length} product${products.length !== 1 ? 's' : ''} listed`}</p>
        </div>
        <Link
      ...
      {!loading && products.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 flex flex-col items-center gap-4 text-center">
          ...
          <p className="font-headline font-bold text-text text-xl">No products yet</p>
          <p className="text-gray-500 text-sm">Start by listing your first product.</p>
          <Link
            to="/artisan/add-product"
